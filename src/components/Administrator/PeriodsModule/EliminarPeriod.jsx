...
const EliminarPeriod = ({ id, numero }) => {
    ...
    const eliminar = async (id) => {
        try {
            ...
            toast.error("Error al intentar eliminar periodo!");
        }
    };

    return (
        <>
        <Tooltip content="Eliminar periodo" placement="right" style="light">
        ...
        <Modal className="bg-black" show={openModal} size="md" onClose={() => setOpenModal(false)} popup>
            ...
            <div className="text-center">
                ...
                <h3 className="mb-5 text-lg font-bold text-primary">
                    ¿Estás seguro de que quieres eliminar el siguiente periodo escolar?
                    <br />
                    <span class="before:block before:absolute before:-inset-1 before:-skew-y-3 before:bg-red-600 relative inline-block">
                        <span class="relative text-white"> periodo: {numero}</span>
                    </span>
                </h3>
                <div className="flex justify-center gap-4">
                <Button color="" className="bg-red-600 font-semibold text-white hover:scale-105 active:scale-95 hover:bg-red-700" onClick={() => { setOpenModal(false); eliminar(id); }}>
                    Sí, estoy seguro
                </Button>
